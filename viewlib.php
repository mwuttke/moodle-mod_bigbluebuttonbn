...
use mod_bigbluebuttonbn\helpers\opencast;
...
function bigbluebuttonbn_view_render(&$bbbsession, $activity) {
    ...
    if ($enabledfeatures['showrecordings'] && $bbbsession['record']) {
        // We check if the Opencast Show Recording config is enabled to show its recordings.
        if (\mod_bigbluebuttonbn\locallib\config::get('opencast_show_recording') && \mod_bigbluebuttonbn\helpers\opencast::bigbluebuttonbn_check_opencast($bbbsession['course']->id)) {
            $output .= html_writer::start_tag('div', array('id' => 'bigbluebuttonbn_view_opencast_recordings'));
            $output .= opencast::bigbluebuttonbn_view_opencast_render_recording_section($bbbsession,  $type);
...
function bigbluebuttonbn_view_render_recordings(&$bbbsession, $enabledfeatures, &$jsvars) {
    ...
    if (empty($ocrecordings) && (empty($recordings) || array_key_exists('messageKey', $recordings))) {
